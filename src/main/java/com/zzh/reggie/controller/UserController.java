...
import com.zzh.reggie.utils.ValidateCodeUtils;
    ...
    public R<User> login(@RequestBody Map map, HttpSession session){
        ...
        if(codeInSession != null && codeInSession.equals(code)){
           ...
           // session.setAttribute("user",user.getId());
            user.setToken(TokenUtils.getToken(user.getId().toString(),user.getPhone()));
            System.out.println("~~~~~~~~~~~~~~~~~~"+TokenUtils.getToken(user.getId().toString(), user.getPhone()));
            //如果用户登录成功删除缓存中验证码
